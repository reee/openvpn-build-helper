...
!define OPENVPN_ROOT   "E:\Dropbox\eduVPN\openvpn-build-helper\pre-bin\x86_64"
...
Section /o "${PACKAGE_NAME} Service" SecService
	...
	; -----------Do Not Use config-README and sample Config---------
...
Section "Uninstall"
	...
	;Delete Our Own README
	Delete "$INSTDIR\doc\eduVPN-README.txt"	
	...
	;------Do Not Delete sample config Because We Do Not Have One-----
	;Delete "$INSTDIR\sample-config\*.${OPENVPN_CONFIG_EXT}"

	;------Delete The One We Insert Ouself------
	Delete "$INSTDIR\config\ca.crt"
	Delete "$INSTDIR\config\ta.key"
	Delete "$INSTDIR\config\*.${OPENVPN_CONFIG_EXT}"
